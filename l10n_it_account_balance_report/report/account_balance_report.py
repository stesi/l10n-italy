...
class ReportAccountBalanceReport(models.TransientModel):
    ...
    def _get_account_line_data(
        ...
        if self.show_partner_details:
            balance = amounts_data[account_id].get("ending_balance") * sign
            ...
            for partner_id in account_partners:
                partner_balance = amounts_data[account_id][partner_id]["ending_balance"]
                partners.append(
                    {
                        "id": partner_id,
    ...
    def get_section_balance_vals(self, data):
        ...
        if currency:
            currency_class = self.env['res.currency']
            if not isinstance(currency, int) and isinstance(currency,type(currency_class)):

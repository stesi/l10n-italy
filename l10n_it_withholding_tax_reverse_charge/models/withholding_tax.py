    ...
    def get_wt_competence(self, amount_reconcile):
        ...
        for st in self:
            if st.invoice_id:
                ...
                if wt_inv:
                    # Aggiunto per fixare il caso di reverse charge con pagamento automatico
                    amount_net_pay = st.invoice_id.amount_net_pay
                    for p in st.invoice_id.payment_id:
                        amount_net_pay -= p.amount

                    amount_base = st.invoice_id.amount_untaxed * (
                        amount_reconcile / amount_net_pay
                    )
